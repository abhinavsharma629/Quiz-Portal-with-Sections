...
def detail(request, section_no, id_no):
	...
	if(request.user.username=='admin'):
		...
		#Quiz Ended
		if(section_no=='1'):
			time=Time1.objects.all()
		elif(section_no=='2'):
			time=Time2.objects.all()
		else:
			time=Time3.objects.all()
		print(time)
...
def adminall(request):
	...
	for j in users:
		if(j.username =='admin'):
			...
			userscore.append(len(SolvedQ1.objects.filter(Q(id_no=j), Q(check=True))))
			userscore.append(len(SolvedQ2.objects.filter(Q(id_no=j), Q(check=True))))
			userscore.append(len(SolvedQ3.objects.filter(Q(id_no=j), Q(check=True))))
...
def DownloadUserData(request):
	...
	writer.writerow(['Enrollment Number', 'Section1 Score', 'Section2 Score', 'Section3 Score'])
